...
# 프로젝트 개요
팀원 누구나 동일하게 실행, 배포, 디버깅할 수 있는 재현 가능한 개발 워크스테이션 환경을 구축하고, 개발의 기초가 되는 도구 활용법을 익히는 프로젝트입니다. 내 컴퓨터에서만 돌아가는 코드 문제를 줄이고, 동일한 서비스를 여러 번 실행해도 일관되게 동작하는 환경을 만드는 것을 목표로 합니다.

# 사용 기술 
- Linux CLI (Terminal): 작업 디렉토리 관리 및 파일 권한 설정
- Docker: 컨테이너 기반 격리된 실행 환경 구축, 웹 서버 컨테이너화
- Git / GitHub: 소스코드 버전 관리 및 협업 기반 관리

# 실습 및 구현 내용
- 환경 세팅 및 터미널 제어: 리눅스 터미널을 이용한 작업 디렉토리 및 권한 정리
- Docker 기초 및 관리: Docker 설치 점검, 컨테이너 실행 및 관리
- 웹 서버 컨테이너화: Dockerfile을 활용한 웹 서버 빌드, 포트 매핑을 통한 접속 확인
- 데이터 영속성 및 변경 반영 검증: 바인드 마운트와 볼륨을 통한 실시간 변경 사항 반영 및 데이터 유지 검증
